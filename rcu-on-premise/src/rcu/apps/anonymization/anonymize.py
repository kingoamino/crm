...
@time_it
def execute_anonymization(client):
    """
    Executes the anonymization for a specific client (project).

    Parameters
    ----------
    client : str
        The client (project) the anonymization will be executed for.
    """

    # Define the metastore directory
    metastore_dir = 'data/' + client + '/metastore/anonymization/'
    ...
    try:
        ...
        conn.rollback()
        logging.exception('--> Rollback de l''anonymsation')
        sys.exit(1)
